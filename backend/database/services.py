...
class ElasticsearchClient(ABC):
    ...
    def __init__(self, index_mapping: dict, log_file: str = 'app.log') -> None:
        ...
        self.base_index_url = urljoin(
            BASE_URL, index_mapping['index_name']) + '/'
...
class ElasticsearchCRUDClient(ElasticsearchClient):
    ...
    def create_index(self, index_name):
        try:
            response = requests.put(
                f'http://localhost:9200/{index_name}', json=self.index_mapping)
    ...
    def get_all_documents(self) -> list:
        try:
            ...
            self.logger.info(
                f'Got all documents from index "{self.index_name}": {documents}')
            return documents
        except requests.exceptions.RequestException as e:
    ...
    def update_document_by_id(self, doc_id: str, updated_fields: dict):
        if not self._validate_document(updated_fields['doc']):
            ...
            updated_fields = self._datetime_validate_document(
                updated_fields['doc'])
            del updated_fields['created_at']
            updated_fields = {'doc': updated_fields}
            try:
                response = requests.post(
                    f'{self.base_index_url}_doc/{doc_id}/_update', json=updated_fields)
                response.raise_for_status()
                self.logger.info(
                    f'Document with ID "{doc_id}" updated successfully')
    ...
    def delete_document_by_id(self, doc_id: str):
        try:
            ...
            response.raise_for_status()
            self.logger.info(
                f'Document with ID "{doc_id}" deleted successfully')
        except requests.exceptions.RequestException as e:
    ...
    def _validate_document(self, document: dict) -> bool:
        ...
        for field in document:
            if field not in self.mapping['mappings']['properties']:
                self.logger.warning(
                    f'Field "{field}" is not defined in the index: {self.index_name})')
    ...
    def search(self, query):
        ...
        try:
            response = requests.get(
                self.search_url, headers=headers, json=body)
            response.raise_for_status()
            results = response.json()
            self.logger.info(
                f'Найденно по запросу: {query} , result: {results}')
            return results
        except requests.exceptions.RequestException as e:
